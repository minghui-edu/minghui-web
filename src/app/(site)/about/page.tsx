...
function SocialButton({ icon, label, color }: { icon: React.ReactNode; label: string; color: string }) {
  return (
    <a
      href="#"
      className={`${color} text-white flex items-center px-4 py-2 rounded-full hover:opacity-90 transition-opacity shadow-sm`}
    >
      <span className="mr-2 w-5 h-5 flex items-center justify-center">{icon}</span>
            ...
            {[
              ...
              <div key={idx} className="flex gap-4">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${item.highlight ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-400'}`}>
                  {item.highlight ? <Star className="w-5 h-5" /> : <div className="w-3 h-3 bg-slate-400 rounded-full" />}
                </div>
